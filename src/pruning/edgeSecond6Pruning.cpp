...
edgeSecond6Pruning::edgeSecond6Pruning()
{
    file_path = "edgeSecond6Pruning.data";

    buildPruneTable();
}

int edgeSecond6Pruning::to_index(Cube const& cube) const {
    ...
    for (int i = 0; i < 6; i++) {
        state = (state << 1) + orient[i + 6];
    }
    return state;
}

int edgeSecond6Pruning::pruning_number(Cube &cube)
{
    return prune_table[to_index(cube)];
}

void edgeSecond6Pruning::buildPruneTable()
{
...
void edgeSecond6Pruning::pruneTreeSearch(Cube & cube, char depth_left, char depth, int lastMove)
{
    int state = to_index(cube);
    if (depth_left == 0)
    {
...
bool edgeSecond6Pruning::solveable(Cube & cube, char depth, char maxBreathDepthSearch, int lastMove)
{
    int state = to_index(cube);
    if (prune_table[state] == depth)
    {
