...
private:
	...
	long long nodeCnt;
	string solution;
	vector<vector<long long> > co_transition, cp_transition, eo_transition, ep_transition;
	vector<char> c_prune, eo_prune, ep1_prune, ep2_prune;
	string moveNames[6], moveCntNames[3];
	unordered_map<long long, char> e_prune;
...
private: 
	bool treeSearch(vector<int> cornerPermutation, vector<int> cornerOrientation, vector<int> edgeOrientation, vector<int> edgePermutation1, vector<int> edgePermutation2, char depth, int lastMove);
