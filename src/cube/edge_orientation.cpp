...
void EdgeOrientation::apply_move(std::vector<int> & arr, int move)
{
    if (move == 0)
    {
...
int EdgeOrientation::array_to_index(std::vector<int> const& arr)
{
    return Indexing::orientation_to_index_dependent(arr, 2);
}

void EdgeOrientation::permute_array(std::vector<int> & arr, int idx1, int idx2, int idx3, int idx4, int increment)
{
    int tmp = arr[idx1];
